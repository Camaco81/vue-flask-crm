<template>
  <div class="credit-payment-container">
    ...
    <div v-if="selectedCustomer" class="modal-overlay" @click.self="closeModalOnOverlay">
      <div class="modal-content">
        <div class="modal-header">
          <h3>Ventas a Crédito - {{ selectedCustomer.name }}</h3>
          <button @click="closeModal" class="close-btn" title="Cerrar ventana">
            &times;
          </button>
        </div>
    ...
    <div v-if="showConfirmation" class="modal-overlay" @click.self="cancelPayment">
...
export default {
  ...
  methods: {
    handleKeydown(event) {
      ...
      if (event.key === 'Escape' && this.selectedCustomer) {
        this.closeModal();
      }
    },
    ...
    closeModal() {
      ...
      if (this.paymentProcessing) {
        const confirmClose = confirm('Hay un pago en proceso. ¿Está seguro de cerrar? Se perderán los datos no guardados.');
        if (!confirmClose) return;
      }

      // Verificar si hay montos ingresados
      const hasPendingPayments = Object.keys(this.paymentAmounts).some(saleId => {
        return this.paymentAmounts[saleId] && parseFloat(this.paymentAmounts[saleId]) > 0;
      });

      if (hasPendingPayments) {
        const confirmClose = confirm('Hay pagos ingresados que no han sido procesados. ¿Está seguro de cerrar?');
        if (!confirmClose) return;
      }
